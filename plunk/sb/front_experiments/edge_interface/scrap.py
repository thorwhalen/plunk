...
from river_t import RiverTrain
...
def main(viz=True):
    ...
    (
        obs.partition(partition_obs)
        .map(concat)
        .accumulate(functools.partial(accumulate, last_lines=backlog_obs))
        .sink(pipe_in.send)
    )
    (clusters.partition(particion_clusters).map(pd.concat).sink(pipe_out.send))
    ...
    if viz:
        ...
        scatter_dmap_input = hv.DynamicMap(hv.Scatter, streams=[pipe_in]).opts(
            color="blue"
        )
        scatter_dmap_output = hv.DynamicMap(hv.Scatter, streams=[pipe_out]).opts(
            color="red"
        )
