...
class StoreExplorer(OutputBase):
    def render(self):
        ...
        print(f'Start {b.render_count()=} {b.depth_keys()=}')

        col1, col2 = st.columns(2)
        with col1:
            st.write('# Keys')
            for i in range(len(b.depth_keys()) + 1):
                if i == 0:
                    d = store
                elif len(b.depth_keys()) == i and (b.depth_keys()[i - 1]) is not None:
                    st.write(f'walrus activated')
                    dk = b.depth_keys()[i - 1]
                    d = d[dk]
                else:
                    break
                ...
                if isinstance(d, dict):
                    ...
                    b.depth_keys = [*b.depth_keys()[:i], k]
                    st.write(f'current {b.depth_keys()=}')
        ...
        print(f'{b.depth_keys()=}')
        print(f'Finish {b.render_count()=}')
        b.render_count = b.render_count() + 1
