...
class OutputNodeAttribute:
    ...
    def submit(self, parent):
        # self.parent_node = dpg.get_value(parent)
        # self.parent_node = dpg.get_item_info(parent)
...
class NodeEditor:
    # @staticmethod
    def _link_callback(self, sender, app_data, user_data):
        ...
        self._links.append((output_attr.parent_node, input_attr.parent_node))
    ...
    def submit(self, parent):
        ...
        with dpg.child_window(
            ...
            with dpg.node_editor(
                tag=self.uuid, callback=self._link_callback, width=-1, height=-1
            ):
...
def view_state(node_editor):
    ...
    #     # print(dir(node))
    #     print(
    #         f"{node.label=}, {node.uuid=}, {node._data=}, {node._output_attributes=}, {node._input_attributes=}"
    #     )
    #     label = node.uuid
    #     if len(node._output_attributes) > 0:
    #         attribute = node._output_attributes[0]
    #         print(
    #             f"{attribute.uuid=}, {attribute._label=}, {attribute._data=},{attribute._children=},{attribute.parent_node=}"
    #         )
    #         # print(dir(attribute))
    #     # print(dpg.get_item_info(label))
    #     break
