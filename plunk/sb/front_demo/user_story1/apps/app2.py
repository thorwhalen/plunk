...
def mk_pipeline_maker_app_with_mall(
    ...
    *,
    ...
    def learn_outlier_model(tagged_data, preprocess_pipeline, n_centroids=15):
        sound, tag = tagged_sound_to_array(*tagged_data)
        wfs = np.array(sound)
        # st.write(wfs[0])

        wfs = assert_dims(wfs)
        # st.write(f"shape={wfs.shape}, ndim ={wfs.ndim}")
